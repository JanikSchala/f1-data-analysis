"""Analyse-Funktionen mit Typannotationen.

bewusst duenne Wrapper um f1lab statt neu geschriebener Rechnung. f1lab ist
in diesem Repository schon die getestete Quelle fuer Race Pace, Degradation
und Stints. App und P-Skripte nutzen sie gemeinsam. hier zaehlt nur, dass
die Bausteine zu einem Werkzeug zusammenkommen.
"""
...
def optimal_plan(session) -> tuple[RaceConfig, Strategy]:
    """exakt bester Boxenstopp-Plan dieser Session. aus echter Degradation
    und echtem Pitloss, keine von Hand gesetzten Zahlen.
...
def overtake_summary(session, quali_session) -> dict:
    """ueberholungen dieser Session und ihr Anteil in der DRS-Zone.
    ``quali_session`` liefert die DRS-Zonen: im Rennen selbst ist oft keine
    offene Zone messbar."""
